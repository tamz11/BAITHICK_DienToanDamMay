    ...
    useEffect(() => {
        ...
        ;(async () => {
            try {
                ...
                const list = (data.categories || []).map(c => ({ name: c.name }))
            ...
            <div className="mx-6">
                <div className="flex items-center justify-between max-w-7xl mx-auto py-4  transition-all">
                    ...
                    <div className="hidden sm:flex items-center gap-4 lg:gap-8 text-slate-600">
                        ...
                        <button onClick={() => setShowContact(true)} className="text-slate-600">Liên hệ</button>
                        ...
                        <form onSubmit={handleSearch} className="hidden xl:flex items-center w-xs text-sm gap-2 bg-slate-100 px-3 py-2 rounded-full">
                            ...
                            <select value={category} onChange={(e) => setCategory(e.target.value)} className="text-sm bg-transparent outline-none px-2 hidden lg:inline">
                                <option value="">Tất cả</option>
                                {categories.map((c) => (
                                    <option key={c.name} value={c.name}>{c.name}</option>
                                ))}
                            </select>
                            <input className="w-full bg-transparent outline-none placeholder-slate-600" type="text" placeholder="Tìm kiếm sản phẩm" value={search} onChange={(e) => setSearch(e.target.value)} />
                        </form>
