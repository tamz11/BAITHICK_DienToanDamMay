...
export async function PUT(req) {
    try {
        ...
        // Nhận phone từ frontend gửi lên
        ...
        // 🌟 TIẾN HÀNH CẬP NHẬT KHỚP 100% TÊN CỘT TRONG SCHEMA CỦA BẠN
        const updatedStore = await prisma.store.update({
            where: { id: store.id },
            data: {
                name: name,
                description: description || "",
                contact: phone || "", // ➔ ĐÃ FIX: Ánh xạ chuẩn biến phone vào cột 'contact' trong DB
                email: email || "",
                address: address || "",
                logo: finalLogoUrl
            }
        ...
        return NextResponse.json({ error: data?.error || "Lỗi hệ thống khi lưu thông tin" }, { status: 500 });
    }
}